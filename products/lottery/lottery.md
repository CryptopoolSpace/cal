...
* Lottery Ticket Fee for 1 ticket: 1 Trust
* Single User Lottery Entry Limit: No limit
* Paying for one ticket \(1 Trust\) will give users a random 4 digit combination with each digit being between 1-10, for e.g. “1-9-3-2”
* In the UI, users can choose to enter a multiple of 10 Trust, which will buy them multiple tickets.
...
* Users will receive a ticket \(comprised of 4 digits\) for each 1 Trust paid.
...
* The 2nd lottery number will be determined based on the %10 remainder of a hash encoded by the blockhash and the total pooled Trust balance at the entry deadline.
